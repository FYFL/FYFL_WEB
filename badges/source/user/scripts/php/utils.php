 ...
 * Throws an ImageUploadException									*/
	...
	//or return the file contents as a string
		return fread(fopen($image['tmp_name'],'r'),$image['size']);
	}
/* Decodes data URIs and outputs them as escaped strings.
 */
	function dataURIImage($image){
		return base64_decode(substr($image,strpos($image,",")+1));
	}
